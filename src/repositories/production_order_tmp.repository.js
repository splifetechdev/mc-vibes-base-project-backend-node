...
exports.findProductionOrderTempAll = async (id) =>
  await db.sequelize.query(
    `SELECT * FROM tbl_opn_tmp
     WHERE tbl_opn_tmp.company_id = :id`,
    {
      replacements: { id },
...
exports.findProductionOrderTempAllByID = async (id) =>
  await db.sequelize.query(
    `SELECT * FROM tbl_opn_tmp
     WHERE tbl_opn_tmp.id = :id`,
    {
      replacements: { id },
      type: db.sequelize.QueryTypes.SELECT,
    }
...
exports.findDueDateByRouting = async (
  ...
  await db.sequelize.query(
    `SELECT opn_end_date_time FROM tbl_opn_tmp
...
exports.findListByCompany = async (company_id) =>
  await db.sequelize.query(
    `SELECT 
        ...
        FROM tbl_opn_tmp
     WHERE tbl_opn_tmp.company_id = :company_id`,
    {
...
exports.create = async (data) => await db.tbl_opn_tmp.create(data);

exports.update = async (doc_running, rtg_id, item_master_id, opn_id, data) =>
  await db.tbl_opn_tmp.update(data, {
    where: {
